...
class Board(object):
    ...
    def sanitycheck_players(self, players):
        for tileno in self.tiles:
            tile = self.tiles[tileno]
            if tile.playersid:
                player_exists = players.player_exists(tile.playersid)
                if not player_exists:
                    print "Player not found, resetting tile", tile.id
                    tile.reset_tile()
